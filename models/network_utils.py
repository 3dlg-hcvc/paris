...
class VanillaMLP(nn.Module):
    ...
    def make_linear(self, dim_in, dim_out, is_first, is_last):
        if self.enable_bias:
            ...
            if self.sphere_init:
                if is_last:
                    torch.nn.init.constant_(layer.bias, -self.sphere_init_radius)  # network without bias will degrade quality
            ...
            layer = nn.Linear(dim_in, dim_out, bias=False) # network without bias will degrade quality
            if self.sphere_init:
                if is_last:
                    torch.nn.init.normal_(layer.weight, mean=math.sqrt(math.pi) / math.sqrt(dim_in), std=0.0001)
                elif is_first:
