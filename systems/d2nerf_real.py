...
from systems.criterions import PSNR, binary_cross_entropy, geodesic_distance, axis_error, SSIM
from systems.utils import parse_optimizer, parse_scheduler, load_gt_axis_real, proj2img, json_quaternion_to_axis_angle, load_gt_info_real
from utils.chamfer import eval_CD, eval_CD_start
from utils.rotation import R_from_axis_angle


@systems.register('d2nerf_real-system')
class D2NeRFRealQuatSystem(BaseSystem):
    """
    ...
    def prepare(self):
        ...
        # metric purpose
        self.gt_info = load_gt_info_real(self.config.model.motion_gt_path)
        rot_diff = geodesic_distance(self.model.quaternions.detach(), self.gt_info['R'])
        print('init rot_diff', rot_diff.item() * 2)
        init_info = {'rot_diff': rot_diff.item() * 2}
        self.save_json('init_axis.json', init_info)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if (self.global_step /2) > self.config.system.loss.reg_from_iter:
            if self.config.system.loss.lambda_blend_ratio > 0.:
                skew = self.config.system.loss.skew_blend_ratio
                loss_ratio = (self.compute_blend_ratio_loss(outs[0]['ratio'], skew=skew) + \
                              self.compute_blend_ratio_loss(outs[1]['ratio'], skew=skew)) * 0.5
            ...
            if (self.config.system.loss.lambda_align_noi > 0.) and ((self.global_step /2) > self.config.system.loss.reg_from_iter + 5000):
                loss_align_s = 0.5 * (outs[0]['align_reg_s'] + outs[1]['align_reg_s'])
                self.log('train/align_reg_s', loss_align_s)
                loss += loss_align_s * self.config.system.loss.lambda_align_noi
                loss_align_d = 0.5 * (outs[0]['align_reg_d'] + outs[1]['align_reg_d'])
                self.log('train/loss_align_d', loss_align_d)
                loss += loss_align_d * self.config.system.loss.lambda_align_noi
                self.log('train/d2a', torch.maximum(outs[0]['d2a'], outs[1]['d2a']))
                self.log('train/max_grid_size', torch.maximum(outs[0]['max_grid_size'], outs[1]['max_grid_size']))

                # if (self.global_step /2) > self.config.system.loss.reg_from_iter + 0:
                #     loss_align_d = 0.5 * (outs[0]['align_reg_d'] + outs[1]['align_reg_d'])
                #     self.log('train/loss_align_d', loss_align_d)
                #     loss += loss_align_d * self.config.system.loss.lambda_align_noi
        ...
        q_mag = torch.linalg.norm(self.model.quaternions.detach())
        # loss_q_norm = torch.abs(q_mag - 1.)
        # loss += loss_q_norm * 0.1
        # self.log('train/loss_q_norm', loss_q_norm)
        self.log('train/quaternion_norm', q_mag)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not self.config.model.ignore_translate:
            transform_json.update({'translation': self.model.translation.detach()})

        # axis_info to draw on the image
        transform_tensor = json_quaternion_to_axis_angle(transform_json)
        axis_o = transform_tensor['axis_o'].unsqueeze(0)
        axis_d = F.normalize(transform_tensor['axis_d'], dim=0).unsqueeze(0)
    ...
    def validation_epoch_end(self, out):
        ...
        if self.trainer.is_global_zero:
            ...
            # rotation difference
            rot_diff = geodesic_distance(out[0]['transform']['quaternions'], self.gt_info['R'])
            self.log('val/rot_diff', rot_diff, prog_bar=True, rank_zero_only=True)
            # axis error
            rot_angular, rot_dist = axis_error(out[0]['pred_axis'].squeeze(0), self.gt_axis)
            self.log('val/rot_angular', rot_angular, prog_bar=True, rank_zero_only=True)
            self.log('val/rot_dist', rot_dist, prog_bar=True, rank_zero_only=True)


            transform_tensor = out[0]['transform']
            transform_tensor = json_quaternion_to_axis_angle(transform_tensor)
            transform_json = {}
    ...
    def test_epoch_end(self, out):
        ...
        if self.trainer.is_global_zero:
            if self.config.mesh_only:
                ...
                transform_tensor = out[0]['transform']
                transform_tensor = json_quaternion_to_axis_angle(transform_tensor)
                self.save_trans_part_mesh(f"it{int(self.global_step/2)}_dynamic_{resolution}_thre{threshold}.obj",
                ...
                # rotation difference
                rot_diff = geodesic_distance(out[0]['transform']['quaternions'], self.gt_info['R'])
                self.log('test/rot_diff', rot_diff, prog_bar=True, rank_zero_only=True)
                # axis error
                rot_angular, rot_dist = axis_error(pred_axis.squeeze(1), self.gt_axis)
                self.log('test/rot_angular', rot_angular, prog_bar=True, rank_zero_only=True)
                self.log('test/rot_dist', rot_dist, prog_bar=True, rank_zero_only=True)
                ...
                with open(self.get_save_path(f'it{int(self.global_step/2)}_metrics_thre{threshold}.txt'), 'w') as f:
                    ...
                    f.write(f'---------- motion ----------\n')
                    f.write(f'axis angular: {rot_angular}\n')
                    f.write(f'axis distance: {rot_dist}\n')
                    f.write(f'geodesic distance: {rot_diff}\n')
                ...
                self.save_axis(f'it{int(self.global_step/2)}_axis.ply', transform_json)
